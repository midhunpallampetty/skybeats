...
import { addItem } from '../../redux/slices/foodSlice';
...
function FoodMenuModal() {
  ...
  const [selectedItems, setSelectedItems] = useState<{ id: number; name: string; price: number }[]>([]);
  ...
  return (
    <div >
      ...
      >
          ...
          />
          <div className="p-4">
              <h3 className="text-lg  text-white font-extrabold">{item.itemName}</h3>
              <p className="text-white font-extrabold text-sm mb-2">${item.price?.toFixed(2)}</p>
              <p className="text-gray-500 text-xs">Available: {item.stock > 0 ? 'Yes' : 'No'}</p>
              <button
                  onClick={() => handleAddItem({ id: item.id, name: item.itemName, price: item.price })}
